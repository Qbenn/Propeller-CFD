...
\clearpage
	...
	Metoda BEMT (\textit{Blade Element Momentum Theory}) łączy opis lokalnego elementu łopaty z bilansem pędu strugi przechodzącej przez tarczę śmigła \cite{glauert,moriarty}. Stanowi połączenie teorii elementu łopatkowego z jednowymiarową teorią pędu. W podejściu tym łopatę śmigła dzieli się wzdłuż promienia na dyskretne odcinki pierścieniowe. Dla każdego z przekrojów wyznacza się obciążenia aerodynamiczne w oparciu o lokalne warunki napływu i charakterystyki zastosowanego profilu, podczas gdy ogólny bilans pędu w tarczy śmigła umożliwia określenie prędkości indukowanych w strudze.
	
	Mimo pominięcia złożonych, trójwymiarowych struktur przepływu, odwzorowywanych w symulacjach CFD, metoda ta pozwala na sprawne oszacowanie rzędu wielkości generowanych sił oraz wyznaczenie podstawowych trendów aerodynamicznych. Dzięki niskim wymaganiom obliczeniowym algorytm BEMT doskonale sprawdza się jako szybkie narzędzie analityczne, a otrzymane nim rezultaty służą w niniejszej pracy jako wstępny punkt odniesienia do walidacji wyników numerycznych ze środowiska ANSYS Fluent.
	...
	\begin{table}[H]\centering
		...
		\begin{tabular}{cccccc}\toprule
			...
			0 & 27,260 & 0,906 & 284,62 & 0,0 & 0,0000 \\
			5 & 25,364 & 1,037 & 325,73 & 38,9 & 0,2000 \\
			10 & 21,394 & 1,072 & 336,63 & 63,6 & 0,4000 \\
			15 & 15,105 & 0,919 & 288,76 & 78,5 & 0,6000 \\
			20 & 7,487 & 0,568 & 178,46 & 83,9 & 0,8000 \\
			25 & -1,505 & -0,035 & -11,06 & 0,0 & 1,0000 \\
			30 & -9,935 & -0,697 & -218,98 & 0,0 & 1,2000 \\\bottomrule
	\end{tabular}\end{table}
	...
	\begin{itemize}
		...
		\item \textbf{Strefa stacjonarna} walec o średnicy \SI{4}{\meter} (8 średnic śmigła) oraz wysokości \SI{5}{\meter} (10 średnic śmigła), obejmujący strefę wirującą, reprezentujący dalekie pole przepływu.
	\end{itemize}
	...
	Wartość minimalnej ortogonalności (0,045) jest niższa od zalecanego progu. Wynika to z przyjętego kompromisu pomiędzy gęstością siatki a czasem obliczeń, ograniczającego liczbę elementów do ok. 1 miliona. Mimo to symulacje wykazały stabilną zbieżność, co świadczy o tym, że lokalnie niższa jakość siatki nie zaburzyła przebiegu obliczeń.
	...
	\begin{itemize}
		\item \textbf{Wlot (Inlet)}: \texttt{Velocity Inlet} z prędkością \(V_\infty = 0, 5, 10, 15, 20, 25, 30\)~\si{\meter\per\second}. Intensywność turbulencji: 5\%, stosunek lepkości turbulentnej: 10.
	...
	Symulację dla każdego punktu pracy przeprowadzono do momentu uzyskania stabilizacji monitorów sił. Przykładowy przebieg residuów oraz monitorów dla prędkości \(V_\infty = 10\)~\si{\meter\per\second} przedstawiono na rysunkach \ref{fig:residuals_cfd} oraz \ref{fig:pressure_cfd}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{residuals_25ms.png}
		\caption{Przebieg residuów dla \(V_\infty = 10\) m/s, \(n = 3000\) RPM.}
		\label{fig:residuals_cfd}
	\end{figure}
	
	Po niecałych 700 iteracjach wszystkie residua osiągnęły stabilny poziom: $\approx 1\times10^{-2}$.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{thrust_25ms.png}
		\caption{Przebieg ciągu (Thrust) dla \(V_\infty = 10\) m/s, \(n = 3000\) RPM.}
	...
	\begin{figure}[H]
		...
		\caption{Średnie ciśnienie statyczne na łopatce dla \(V_\infty = 10\) m/s.}
		\label{fig:pressure_cfd}
	\end{figure}
	
	Średnie ciśnienie statyczne na powierzchni łopatki (rys. \ref{fig:pressure_cfd}) ustabilizowało się na poziomie ok. \SI{-150}{\pascal}, co jest wartością fizycznie poprawną.
	
	\subsection{Wyniki symulacji CFD}
	
	\subsubsection{Charakterystyka ciągu}
	% UWAGA: wiersz "0 m/s" w tabelach wynikowych CFD odpowiada symulacji wykonanej
	% dla 1 m/s (symulacja punktu statycznego nie zostala zrealizowana).
	...
	Ciąg śmigła maleje monotonicznie wraz ze wzrostem prędkości napływu. W warunkach statycznych (\(V_\infty = 0\) m/s) śmigło wytwarza największy ciąg (17,70 N), który przy \(V_\infty = 20\) m/s spada do 2,74 N, a powyżej tej prędkości zmienia znak na ujemny. Przejście w stan wiatrakowania (tryb hamulcowy) dla \(V_\infty \ge 25\) m/s jest fizycznie poprawnym zachowaniem śmigła i zostało odtworzone przez symulację.
	...
	Moment obrotowy rośnie w zakresie małych prędkości napływu i osiąga maksimum przy \(V_\infty = 10\) m/s (0,80 Nm), po czym maleje wraz ze wzrostem prędkości. Wartości dla pełnego śmigła wyznaczono jako dwukrotność niezaokrąglonych wyników modelu połówkowego, dlatego zaokrąglona wartość połówki może nie być dokładnie równa połowie wartości całkowitej (np. \(-0{,}017\) Nm $\times$ 2 a \(-0{,}033\) Nm). W miarę przechodzenia śmigła w stan hamowania jego wartość spada poniżej zera. Wraz ze spadkiem momentu obrotowego zmniejsza się również moc potrzebna do napędu śmigła.
	...
	Sprawność śmigła osiąga maksimum w zakresie średnich prędkości napływu i gwałtownie maleje przy prędkościach skrajnych. Największą uzyskaną sprawność, wynoszącą ok. 64,0\%, zaobserwowano dla \(V_\infty = 15\) m/s. Dla prędkości powyżej \(V_\infty = 20\) m/s, gdy śmigło przechodzi w stan hamowania (moc ujemna), sprawność spada do zera.
	...
	W tabeli \ref{tab:comparison} zestawiono wyniki metody BMT oraz symulacji CFD dla ciągu i momentu obrotowego wraz z błędem względnym liczonym względem wartości referencyjnej BMT. Ujemne wartości błędu oznaczają, że wynik CFD jest niższy od wyniku BMT, natomiast dodatnie wskazują na wartość wyższą. Tabela \ref{tab:errors_comparison} zawiera błędy względne mocy i sprawności.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}c c c c c c c@{}}
			...
			\(V_\infty\) & \(T_{\text{BMT}}\) & \(T_{\text{CFD}}\) & \(\delta T\) & \(Q_{\text{BMT}}\) & \(Q_{\text{CFD}}\) & \(\delta Q\) \\
 			{[m/s]} & {[N]} & {[N]} & {[\%]} & {[Nm]} & {[Nm]} & {[\%]} \\
			\midrule
			0  & 27,260  & 17,700  & -35,1  & 0,906  & 0,580  & -36,0  \\
			5  & 25,364  & 14,480  & -42,9  & 1,037  & 0,640  & -38,3 \\
			10 & 21,394  & 12,738  & -40,5  & 1,072  & 0,800  & -25,4 \\
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}c c c c c@{}}
			...
			{[m/s]} & {[\%]} & {[\%]} & {[\%]} & {[\%]} \\
			\midrule
			0  & -36,0   & -35,1  & -36,0   & 0,0   \\
			5  & -38,3  & -42,9  & -38,3  & -7,4  \\
			10 & -25,4  & -40,5  & -25,4  & -20,3 \\
			15 & -28,2  & -41,5  & -28,2  & -18,5 \\
			20 & -26,1  & -63,4  & -26,1  & -50,5 \\
			25 & +5,4   & -165,9 & +4,8   & --    \\
			30 & +71,5  & +6,6   & +71,5  & --    \\
			\bottomrule
		\end{tabular}
	...
	Dla prędkości \(V_\infty = 30\) m/s, gdy śmigło pracuje w trybie hamulcowym (ciąg ujemny), wyniki CFD porównano z obliczeniami BMT. Uzyskana różnica względna wynosi ok. \(+6{,}6\%\) (CFD: \(-9{,}28\) N, BMT: \(-9{,}94\) N), co stanowi dobrą zgodność obu metod w tym reżimie przepływu. W zakresie małych i średnich prędkości napływu symulacja CFD przewiduje niższe wartości ciągu niż metoda BMT, a rozbieżności względne zawierają się w przedziale ok. 35--63\%. Wynika to z uproszczeń modelu analitycznego oraz zjawisk trójwymiarowych, których metoda BMT nie uwzględnia. Skok błędu względnego do ok. \(-166\%\) przy \(V_\infty = 25\) m/s wynika z przejścia przez zero ciągu BMT (bardzo mały mianownik błędu), a nie z rzeczywistego wzrostu rozbieżności bezwzględnej.
	...
	\begin{itemize}
		\item \textbf{Uproszczenia metody BMT}: metoda elementów łopaty i pędu opiera się na założeniu dwuwymiarowego przepływu na każdym przekroju łopaty, nie uwzględnia w pełni efektów trójwymiarowych, takich jak przepływy promieniowe, strefy zwartego wiru końcowego oraz interakcje łopaty ze strugą. W rezultacie metoda BMT często zawyża ciąg i moment w zakresie małych prędkości napływu, co odpowiada obserwowanemu tu ujemnemu błędowi względnemu CFD (35--63\% mniejsze wartości w zakresie 0--20 m/s).
	...
	\begin{enumerate}
		...
		\item Zaobserwowano, że dla prędkości \(V_\infty > 20\) m/s śmigło przechodzi w tryb hamulcowy (ciąg ujemny), co jest zjawiskiem fizycznie poprawnym i zostało odwzorowane przez obie metody. Punkt zerowego ciągu wyznaczony metodą BEMT (\(\approx 24\) m/s) i CFD (\(\approx 22\) m/s) różni się o ok. 2 m/s.
		\item Największa sprawność śmigła, wynosząca ok. 64,0\%, została osiągnięta dla prędkości napływu 15 m/s. W zakresie małych i średnich prędkości napływu symulacja CFD przewiduje niższe wartości ciągu niż metoda BMT (o ok. 35--63\%), co wynika z uproszczeń modelu analitycznego oraz efektów trójwymiarowych nieuwzględnianych przez BMT.
